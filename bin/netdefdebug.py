    ...
    for lan in lan_list:
        ...
        for item in lan:
            ...
            if lan_def == {}:
                ...
                print "Error:Node[%s] connected with other nodes with "\
        ...
        for item in lan:
            ...
            for npart in nlist:
                ...
                if npart.startswith('NAT:'):
                    print 'vm_net_from_lan_list [%s]' % npart
                ...
                if npart.find('bridge') != -1:
                    ...
                    if not validate_vm_bridge_ip(port_ip, vm_conf):
                        print "Warn: VM[%s]'s bridge IP[%s] doesn't in the same "\
                            "subnet with node." % (vm_conf['Name'], port_ip)
                else:
                    port_ip = npart
                if (port_ip != '') and (not validate_ip(port_ip)):
                    print "Error:" + "The port annotation [%s] should"\
...
def add_vm2node(vm_conf, node):
    ...
    if vm_conf['connect2'] != {}:
        if vm_conf['connect2']['id'] == node['id']:
            ...
            return
        print "Error:" + "VM is only allowed to connect to one "\
    ...
    if tmpvm != {}:
        # Already added.
        print "Warning:Multi-connection between [%s] and [%s].\n" %\
...
def set_port_ip(tip, port_id, port_list):
    ...
    for port in port_list:
        if port['id'] == port_id:
            ...
            found = True
    if not found:
        print "Error:" + "JSON file corrupt? annotation[%s]for no port.\n" % tip
...
def handle_annotation(annotation, conf):
        ...
        if npart.startswith('install.'):
            ...
            if conf["Name"] == '':
                if not validate_str(npart):
                    print "Error: Annotation[%s] only is allowed maxim %s char"\
                        "and only include alpha/number."%(npart, MAX_ANNOTATION_LEN)
                conf["Name"] = npart
            else:
                print "Error: can not recognize [%s] from annotation[%s]" %\
                    (npart, annotation)
                print conf
    if cmd_list != []:
        conf['run'] = cmd_list
    ...
    if conf["Name"] == '':
        print "Error:" + 'Every item [%s] need a name annotation.\n' % annotation
    return conf
        ...
        if npart.startswith('reserve:'):
            ...
            if name == '':
                if not validate_str(npart):
                    ...
                    print "Error: experiment name[%s] should be experiment.team. annotation[%s]" % \
                          (npart, annotation)
            else:
                print "Error: cloud annotation can only have one experiment name [%s] from annotation[%s]" % (npart, annotation)
...
def get_item_from_draw(net_draw):
    ...
    if not net_draw.has_key("figures"):
        print "Error:" + "Input file is corrupt for it doesn't have key 'figures'.\n"
        return [], [], [], ''
    for figure in net_draw['figures']:
        ...
        if ftype == NODE_TYPE:
            ...
            print "Error:" + "Unknown icon type [%s],please only use the following icons: %s" % \
    ...
    for cloud in cloud_list:
        for port in cloud['ports']:
            for connection in port['connections']:
                ...
                if node == {}:
                    print "Error: Cloud Icon can only connect to nodes. Only "\
                    "nodes can access the internet."
                    continue
                node['Type'] = 'Gateway'
                if tip != '':
                    print "Warn: IP(%s) of the port connectted to cloud is "\
        ...
        for port in node['ports']:
            for connection in port['connections']:
                if port['id'] != connection['source']:
                    print "Error:" + "Router[%s] port[%s] connection source[%s] is "\
                ...
                if cloud != {}:
                    ...
                    print "Error: Node[%s]'s connection from id[%d] is not to"\
        ...
        for port in conf['ports']:
            ...
            for connection in port['connections']:
                if port_id != connection['source']:
                    print "Error:" + "vm[%s] port[%s] connection source[%s] is not "\
                ...
                (rvm, tip) = get_item_by_portid(vm_list, tid)
                if rvm == '':
                    print "Error: VM[%s]'s connection from port id[%d] can "\
                ...
                if port.has_key('IP'):
                    ...
                    port_ip = ''
                    print 'Warn: No IP at the port from VM[%s] to VM[%s]' % \
                        (name, rvm['Name'])
...
def draw2netdef(net_draw):
    ...
    for vm_conf in vm_list:
        if vm_conf['connect2'] == {}:
            print "Error:" + "VM[%s] need connect to a router.\n" %(vm_conf['Name'])
...
def test_link():
    ...
    vm_list = [{'Name':'v1','id':'v1', 'connect2':{}, 'ports':[{'id':'v1p1', 'connections':[{'target':'v2p1'}]}]},
...
def test():
    ...
    net_draw = {"figures":[{"id":"4onRN0sott", "type":"DiagramFigure", "subtype":"asa", "pic":"icons/asa.png", "x":472, "y":342, "width":59, "height":66, "ports":[{"connections":[{"source":"CXOEGoZk1E", "target":"5KkffMqtfC", "color":"128,128,255"}], "x":0, "y":33, "id":"CXOEGoZk1E"},{"connections":[], "x":29.5, "y":0, "id":"ZPWm1bsTle"},{"connections":[], "x":59, "y":33, "id":"5xmlUfGKX1"},{"connections":[], "x":29.5, "y":66, "id":"7AiJDaOtTa"},{"connections":[], "x":59, "y":66, "id":"iMXMA5weDZ"},{"connections":[], "x":0, "y":66, "id":"T8zbzuDPsa"},{"connections":[], "x":59, "y":0, "id":"fekBNGnJ9o"},{"connections":[], "x":0, "y":0, "id":"pSctSd0hVR"}], "annotations":[{"text":"n1", "x":473, "y":413, "for":"4onRN0sott"}]},{"id":"0O0VITUXG4", "type":"DiagramFigure", "subtype":"cloud_filled", "pic":"icons/cloud_filled.png", "x":642, "y":345, "width":84, "height":56, "ports":[{"connections":[{"source":"Cl0CqB22cD", "target":"5xmlUfGKX1", "color":"128,128,255"}], "x":0, "y":28, "id":"Cl0CqB22cD"},{"connections":[], "x":42, "y":0, "id":"JMAbzV4yqu"},{"connections":[], "x":84, "y":28, "id":"9p9T5GfL7K"},{"connections":[], "x":42, "y":56, "id":"NHg9JSOs4U"},{"connections":[], "x":84, "y":56, "id":"Mp6HWtwJtz"},{"connections":[], "x":0, "y":56, "id":"oRUzrZTqll"},{"connections":[], "x":84, "y":0, "id":"ofPz77ZIop"},{"connections":[], "x":0, "y":0, "id":"px5tWoH1ww"}], "annotations":[{"text":"exp.team", "x":643, "y":406, "for":"0O0VITUXG4"}]},{"id":"quEq5MLf8v", "type":"DiagramFigure", "subtype":"router", "pic":"icons/router.png", "x":283, "y":331, "width":66, "height":45, "ports":[{"connections":[], "x":0, "y":22.5, "id":"BLW5tQBTdf"},{"connections":[], "x":33, "y":0, "id":"TENAonPT4P"},{"connections":[], "x":66, "y":22.5, "id":"5KkffMqtfC"},{"connections":[], "x":33, "y":45, "id":"0ozlNxRzdM"},{"connections":[], "x":66, "y":45, "id":"3UsByDne5x"},{"connections":[], "x":0, "y":45, "id":"wsMTmJ4LRM"},{"connections":[], "x":66, "y":0, "id":"TUOUqfbp7R"},{"connections":[], "x":0, "y":0, "id":"gvzmLuFmrC"}], "annotations":[{"text":"routern1; forward", "x":284, "y":381, "for":"quEq5MLf8v"},{"text":"", "x":349, "y":353.5, "for":"5KkffMqtfC"}]}]}
    netdef = draw2netdef(net_draw)
    assert netdef == {
    ...
    assert netdef == {"Nodes": [{"Name": "n1"}, {"Name": "n2"}],
                      "LANs": [{"Subnet": "172.16.1.",
                                "LAN": [["n2", "2"], ["n1", "1"]]}]}
    import vagrantfiledebug
    vagrantfiledebug.build_exp_nsfile(netdef)
    ...
    assert vagrantfiledebug.build_exp_nsfile(netdef) == '''set ns [new Simulator]
...
'''
    ...
    net_draw = {"figures": [{
        ...
        "ports": [{"connections": [], "id": "n1p1"},
                  {"connections": [{"source":"n1p2", "target":"v1n1p1"}],
                   "id": "n1p2"},
                  {"connections": [], "id": "n1p3"},
                  {"connections": [], "id": "n1p4"}]}, {
        "subtype": "workstation",
    ...
    json_diagram = {"figures":[{"id":"lEfpZPDAcJ", "type":"DiagramFigure", "subtype":"asa", "pic":"icons/asa.png", "x":981, "y":298, "width":59, "height":66, "ports":[{"connections":[{"source":"TzLzMdBsVm", "target":"dCQykgLqKM", "color":"128,128,255"}], "x":0, "y":33, "id":"TzLzMdBsVm"},{"connections":[], "x":29.5, "y":0, "id":"K0qJfIPe7I"},{"connections":[], "x":59, "y":33, "id":"VPNWWLfPJf"},{"connections":[], "x":29.5, "y":66, "id":"c9IXpXDFQ0"}], "annotations":[{"text":"n1", "x":982, "y":369, "for":"lEfpZPDAcJ"}]},{"id":"AIbVnDVrVC", "type":"DiagramFigure", "subtype":"cloud_filled", "pic":"icons/cloud_filled.png", "x":1112, "y":303, "width":84, "height":56, "ports":[{"connections":[{"source":"SwdUa2ZF3p", "target":"VPNWWLfPJf", "color":"128,128,255"}], "x":0, "y":28, "id":"SwdUa2ZF3p"},{"connections":[], "x":42, "y":0, "id":"8XwVq6tcZe"},{"connections":[], "x":84, "y":28, "id":"u83rLhZAZJ"},{"connections":[], "x":42, "y":56, "id":"JcmIX58xv0"}], "annotations":[{"text":"e1.teamname", "x":1113, "y":364, "for":"AIbVnDVrVC"}]},{"id":"waSTPwlodp", "type":"DiagramFigure", "subtype":"web_server", "pic":"icons/web_server.png", "x":467, "y":249, "width":53, "height":55, "ports":[{"connections":[], "x":0, "y":27.5, "id":"waC24D9HMF"},{"connections":[], "x":26.5, "y":0, "id":"kbPPhzawpr"},{"connections":[], "x":53, "y":27.5, "id":"KvfzIPL1yF"},{"connections":[], "x":26.5, "y":55, "id":"NKxLHWaVmP"}], "annotations":[{"text":"win10", "x":468, "y":309, "for":"waSTPwlodp"},{"text":"172.16.1.2", "x":520, "y":276.5, "for":"KvfzIPL1yF"}]},{"id":"AkbPLVpc4r", "type":"DiagramFigure", "subtype":"workstation", "pic":"icons/workstation.png", "x":455, "y":344, "width":64, "height":53, "ports":[{"connections":[], "x":0, "y":26.5, "id":"DhaDHbG63n"},{"connections":[], "x":32, "y":0, "id":"h4k2uhH4LG"},{"connections":[], "x":64, "y":26.5, "id":"SfbuEm5U3Z"},{"connections":[], "x":32, "y":53, "id":"XgrMfguCLx"}], "annotations":[{"text":"ubuntu", "x":456, "y":402, "for":"AkbPLVpc4r"},{"text":"172.16.1.3", "x":519, "y":370.5, "for":"SfbuEm5U3Z"}]},{"id":"3IIAhJS92D", "type":"DiagramFigure", "subtype":"workstation", "pic":"icons/workstation.png", "x":633, "y":504, "width":64, "height":53, "ports":[{"connections":[], "x":0, "y":26.5, "id":"sy76SdLSqB"},{"connections":[], "x":32, "y":0, "id":"JLFU7ZdP7P"},{"connections":[], "x":64, "y":26.5, "id":"lmILTLbFGq"},{"connections":[], "x":32, "y":53, "id":"B0AprTpMTO"}], "annotations":[{"text":"db; install.db", "x":634, "y":562, "for":"3IIAhJS92D"},{"text":"172.16.3.2", "x":665, "y":504, "for":"JLFU7ZdP7P"}]},{"id":"5rbdmr84Q0", "type":"DiagramFigure", "subtype":"workstation", "pic":"icons/workstation.png", "x":762, "y":505, "width":64, "height":53, "ports":[{"connections":[], "x":0, "y":26.5, "id":"p1l5PSKmzI"},{"connections":[], "x":32, "y":0, "id":"khu8TurK0L"},{"connections":[], "x":64, "y":26.5, "id":"S4pJe89i0d"},{"connections":[], "x":32, "y":53, "id":"TMlV4xKdpq"}], "annotations":[{"text":"siem; install.splunk", "x":763, "y":563, "for":"5rbdmr84Q0"},{"text":"172.16.3.3", "x":794, "y":505, "for":"khu8TurK0L"}]},{"id":"eCnmGlXr2t", "type":"DiagramFigure", "subtype":"workstation", "pic":"icons/workstation.png", "x":592, "y":108, "width":64, "height":53, "ports":[{"connections":[], "x":0, "y":26.5, "id":"ML8I5E7bAh"},{"connections":[], "x":32, "y":0, "id":"aNhVmV58Ze"},{"connections":[], "x":64, "y":26.5, "id":"22g6dRqTAA"},{"connections":[], "x":32, "y":53, "id":"FXeMW4RIZQ"}], "annotations":[{"text":"web; install.web", "x":593, "y":166, "for":"eCnmGlXr2t"},{"text":"172.16.2.2", "x":624, "y":161, "for":"FXeMW4RIZQ"}]},{"id":"Fdd4V6Qoyf", "type":"DiagramFigure", "subtype":"workstation", "pic":"icons/workstation.png", "x":690, "y":110, "width":64, "height":53, "ports":[{"connections":[], "x":0, "y":26.5, "id":"BEeeKQhvlM"},{"connections":[], "x":32, "y":0, "id":"Bg6yRqsANB"},{"connections":[], "x":64, "y":26.5, "id":"sWI6oW7kx0"},{"connections":[], "x":32, "y":53, "id":"lxTUSJSm6D"}], "annotations":[{"text":"email", "x":691, "y":168, "for":"Fdd4V6Qoyf"},{"text":"172.16.2.3", "x":722, "y":163, "for":"lxTUSJSm6D"}]},{"id":"q0lbQJLT6G", "type":"DiagramFigure", "subtype":"workstation", "pic":"icons/workstation.png", "x":777, "y":108, "width":64, "height":53, "ports":[{"connections":[], "x":0, "y":26.5, "id":"2Vp6MPP0Cn"},{"connections":[], "x":32, "y":0, "id":"RlCnwlc7fT"},{"connections":[], "x":64, "y":26.5, "id":"a9Z6cdqXHS"},{"connections":[], "x":32, "y":53, "id":"7iDKoGfxyq"}], "annotations":[{"text":"dns", "x":778, "y":166, "for":"q0lbQJLT6G"},{"text":"172.16.2.4", "x":809, "y":161, "for":"7iDKoGfxyq"}]},{"id":"zDFyeWPdmD", "type":"DiagramFigure", "subtype":"router", "pic":"icons/router.png", "x":687, "y":309, "width":66, "height":45, "ports":[{"connections":[{"source":"06Acbal21m", "target":"KvfzIPL1yF", "color":"128,128,255"},{"source":"06Acbal21m", "target":"SfbuEm5U3Z", "color":"128,128,255"}], "x":0, "y":22.5, "id":"06Acbal21m"},{"connections":[{"source":"Alun6vU20a", "target":"FXeMW4RIZQ", "color":"128,128,255"},{"source":"Alun6vU20a", "target":"lxTUSJSm6D", "color":"128,128,255"},{"source":"Alun6vU20a", "target":"7iDKoGfxyq", "color":"128,128,255"}], "x":33, "y":0, "id":"Alun6vU20a"},{"connections":[{"source":"oPAUnksBvL", "target":"mL9v2Uyvo1", "color":"128,128,255"}], "x":66, "y":22.5, "id":"oPAUnksBvL"},{"connections":[{"source":"2KEiLTRGLR", "target":"JLFU7ZdP7P", "color":"128,128,255"},{"source":"2KEiLTRGLR", "target":"khu8TurK0L", "color":"128,128,255"}], "x":33, "y":45, "id":"2KEiLTRGLR"}], "annotations":[{"text":"gw; install.snort 172.16.3.3", "x":688, "y":359, "for":"zDFyeWPdmD"},{"text":"172.16.1.100", "x":636, "y":344, "for":"06Acbal21m"},{"text":"172.16.2.100", "x":720, "y":309, "for":"Alun6vU20a"},{"text":"172.16.10.2", "x":753, "y":331.5, "for":"oPAUnksBvL"}]},{"id":"gLW5xKF2Oo", "type":"DiagramFigure", "subtype":"router", "pic":"icons/router.png", "x":863, "y":309, "width":66, "height":45, "ports":[{"connections":[], "x":0, "y":22.5, "id":"mL9v2Uyvo1"},{"connections":[], "x":33, "y":0, "id":"P6RlbrJ9of"},{"connections":[], "x":66, "y":22.5, "id":"dCQykgLqKM"},{"connections":[], "x":33, "y":45, "id":"8fttHOi8xq"}], "annotations":[{"text":"172.16.10.100", "x":863, "y":309, "for":"mL9v2Uyvo1"},{"text":"172.16.20.100", "x":896, "y":354, "for":"8fttHOi8xq"},{"text":"gw2", "x":864, "y":359, "for":"gLW5xKF2Oo"}]},{"id":"cr9dnaTQdN", "type":"DiagramFigure", "subtype":"laptop", "pic":"icons/laptop.png", "x":871, "y":465, "width":85, "height":64, "ports":[{"connections":[], "x":0, "y":32, "id":"UACANDsxWL"},{"connections":[{"source":"BSlivpgzWn", "target":"8fttHOi8xq", "color":"128,128,255"}], "x":42.5, "y":0, "id":"BSlivpgzWn"},{"connections":[], "x":85, "y":32, "id":"RtJKoO7LO2"},{"connections":[], "x":42.5, "y":64, "id":"OFfF9m3dlQ"}], "annotations":[{"text":"172.16.20.2", "x":913.5, "y":465, "for":"BSlivpgzWn"},{"text":"kali", "x":872, "y":534, "for":"cr9dnaTQdN"}]}]}
    ...
    json_diagram = {"figures":[{"id":"lEfpZPDAcJ", "type":"DiagramFigure", "subtype":"asa", "pic":"icons/asa.png", "x":981, "y":298, "width":59, "height":66, "ports":[{"connections":[{"source":"TzLzMdBsVm", "target":"dCQykgLqKM", "color":"128,128,255"}], "x":0, "y":33, "id":"TzLzMdBsVm"},{"connections":[], "x":29.5, "y":0, "id":"K0qJfIPe7I"},{"connections":[], "x":59, "y":33, "id":"VPNWWLfPJf"},{"connections":[], "x":29.5, "y":66, "id":"c9IXpXDFQ0"}], "annotations":[{"text":"n1", "x":982, "y":369, "for":"lEfpZPDAcJ"}]},{"id":"AIbVnDVrVC", "type":"DiagramFigure", "subtype":"cloud_filled", "pic":"icons/cloud_filled.png", "x":1112, "y":303, "width":84, "height":56, "ports":[{"connections":[{"source":"SwdUa2ZF3p", "target":"VPNWWLfPJf", "color":"128,128,255"}], "x":0, "y":28, "id":"SwdUa2ZF3p"},{"connections":[], "x":42, "y":0, "id":"8XwVq6tcZe"},{"connections":[], "x":84, "y":28, "id":"u83rLhZAZJ"},{"connections":[], "x":42, "y":56, "id":"JcmIX58xv0"}], "annotations":[{"text":"e1.teamname; reserve: pc3a, pc4b, pc17e, pc19d", "x":1113, "y":364, "for":"AIbVnDVrVC"}]},{"id":"waSTPwlodp", "type":"DiagramFigure", "subtype":"web_server", "pic":"icons/web_server.png", "x":467, "y":249, "width":53, "height":55, "ports":[{"connections":[], "x":0, "y":27.5, "id":"waC24D9HMF"},{"connections":[], "x":26.5, "y":0, "id":"kbPPhzawpr"},{"connections":[], "x":53, "y":27.5, "id":"KvfzIPL1yF"},{"connections":[], "x":26.5, "y":55, "id":"NKxLHWaVmP"}], "annotations":[{"text":"win10", "x":468, "y":309, "for":"waSTPwlodp"},{"text":"172.16.1.2", "x":520, "y":276.5, "for":"KvfzIPL1yF"}]},{"id":"AkbPLVpc4r", "type":"DiagramFigure", "subtype":"workstation", "pic":"icons/workstation.png", "x":455, "y":344, "width":64, "height":53, "ports":[{"connections":[], "x":0, "y":26.5, "id":"DhaDHbG63n"},{"connections":[], "x":32, "y":0, "id":"h4k2uhH4LG"},{"connections":[], "x":64, "y":26.5, "id":"SfbuEm5U3Z"},{"connections":[], "x":32, "y":53, "id":"XgrMfguCLx"}], "annotations":[{"text":"ubuntu", "x":456, "y":402, "for":"AkbPLVpc4r"},{"text":"172.16.1.3", "x":519, "y":370.5, "for":"SfbuEm5U3Z"}]},{"id":"3IIAhJS92D", "type":"DiagramFigure", "subtype":"workstation", "pic":"icons/workstation.png", "x":633, "y":504, "width":64, "height":53, "ports":[{"connections":[], "x":0, "y":26.5, "id":"sy76SdLSqB"},{"connections":[], "x":32, "y":0, "id":"JLFU7ZdP7P"},{"connections":[], "x":64, "y":26.5, "id":"lmILTLbFGq"},{"connections":[], "x":32, "y":53, "id":"B0AprTpMTO"}], "annotations":[{"text":"db; install.db", "x":634, "y":562, "for":"3IIAhJS92D"},{"text":"172.16.3.2", "x":665, "y":504, "for":"JLFU7ZdP7P"}]},{"id":"5rbdmr84Q0", "type":"DiagramFigure", "subtype":"workstation", "pic":"icons/workstation.png", "x":762, "y":505, "width":64, "height":53, "ports":[{"connections":[], "x":0, "y":26.5, "id":"p1l5PSKmzI"},{"connections":[], "x":32, "y":0, "id":"khu8TurK0L"},{"connections":[], "x":64, "y":26.5, "id":"S4pJe89i0d"},{"connections":[], "x":32, "y":53, "id":"TMlV4xKdpq"}], "annotations":[{"text":"siem; install.splunk", "x":763, "y":563, "for":"5rbdmr84Q0"},{"text":"172.16.3.3", "x":794, "y":505, "for":"khu8TurK0L"}]},{"id":"eCnmGlXr2t", "type":"DiagramFigure", "subtype":"workstation", "pic":"icons/workstation.png", "x":592, "y":108, "width":64, "height":53, "ports":[{"connections":[], "x":0, "y":26.5, "id":"ML8I5E7bAh"},{"connections":[], "x":32, "y":0, "id":"aNhVmV58Ze"},{"connections":[], "x":64, "y":26.5, "id":"22g6dRqTAA"},{"connections":[], "x":32, "y":53, "id":"FXeMW4RIZQ"}], "annotations":[{"text":"web; install.web", "x":593, "y":166, "for":"eCnmGlXr2t"},{"text":"172.16.2.2", "x":624, "y":161, "for":"FXeMW4RIZQ"}]},{"id":"Fdd4V6Qoyf", "type":"DiagramFigure", "subtype":"workstation", "pic":"icons/workstation.png", "x":690, "y":110, "width":64, "height":53, "ports":[{"connections":[], "x":0, "y":26.5, "id":"BEeeKQhvlM"},{"connections":[], "x":32, "y":0, "id":"Bg6yRqsANB"},{"connections":[], "x":64, "y":26.5, "id":"sWI6oW7kx0"},{"connections":[], "x":32, "y":53, "id":"lxTUSJSm6D"}], "annotations":[{"text":"email", "x":691, "y":168, "for":"Fdd4V6Qoyf"},{"text":"172.16.2.3", "x":722, "y":163, "for":"lxTUSJSm6D"}]},{"id":"q0lbQJLT6G", "type":"DiagramFigure", "subtype":"workstation", "pic":"icons/workstation.png", "x":777, "y":108, "width":64, "height":53, "ports":[{"connections":[], "x":0, "y":26.5, "id":"2Vp6MPP0Cn"},{"connections":[], "x":32, "y":0, "id":"RlCnwlc7fT"},{"connections":[], "x":64, "y":26.5, "id":"a9Z6cdqXHS"},{"connections":[], "x":32, "y":53, "id":"7iDKoGfxyq"}], "annotations":[{"text":"dns", "x":778, "y":166, "for":"q0lbQJLT6G"},{"text":"172.16.2.4", "x":809, "y":161, "for":"7iDKoGfxyq"}]},{"id":"zDFyeWPdmD", "type":"DiagramFigure", "subtype":"router", "pic":"icons/router.png", "x":687, "y":309, "width":66, "height":45, "ports":[{"connections":[{"source":"06Acbal21m", "target":"KvfzIPL1yF", "color":"128,128,255"},{"source":"06Acbal21m", "target":"SfbuEm5U3Z", "color":"128,128,255"}], "x":0, "y":22.5, "id":"06Acbal21m"},{"connections":[{"source":"Alun6vU20a", "target":"FXeMW4RIZQ", "color":"128,128,255"},{"source":"Alun6vU20a", "target":"lxTUSJSm6D", "color":"128,128,255"},{"source":"Alun6vU20a", "target":"7iDKoGfxyq", "color":"128,128,255"}], "x":33, "y":0, "id":"Alun6vU20a"},{"connections":[{"source":"oPAUnksBvL", "target":"mL9v2Uyvo1", "color":"128,128,255"}], "x":66, "y":22.5, "id":"oPAUnksBvL"},{"connections":[{"source":"2KEiLTRGLR", "target":"JLFU7ZdP7P", "color":"128,128,255"},{"source":"2KEiLTRGLR", "target":"khu8TurK0L", "color":"128,128,255"}], "x":33, "y":45, "id":"2KEiLTRGLR"}], "annotations":[{"text":"gw; install.snort 172.16.3.3", "x":688, "y":359, "for":"zDFyeWPdmD"},{"text":"172.16.1.100", "x":636, "y":344, "for":"06Acbal21m"},{"text":"172.16.2.100", "x":720, "y":309, "for":"Alun6vU20a"},{"text":"172.16.10.2", "x":753, "y":331.5, "for":"oPAUnksBvL"}]},{"id":"gLW5xKF2Oo", "type":"DiagramFigure", "subtype":"router", "pic":"icons/router.png", "x":863, "y":309, "width":66, "height":45, "ports":[{"connections":[], "x":0, "y":22.5, "id":"mL9v2Uyvo1"},{"connections":[], "x":33, "y":0, "id":"P6RlbrJ9of"},{"connections":[], "x":66, "y":22.5, "id":"dCQykgLqKM"},{"connections":[], "x":33, "y":45, "id":"8fttHOi8xq"}], "annotations":[{"text":"172.16.10.100", "x":863, "y":309, "for":"mL9v2Uyvo1"},{"text":"172.16.20.100", "x":896, "y":354, "for":"8fttHOi8xq"},{"text":"gw2", "x":864, "y":359, "for":"gLW5xKF2Oo"}]},{"id":"cr9dnaTQdN", "type":"DiagramFigure", "subtype":"laptop", "pic":"icons/laptop.png", "x":871, "y":465, "width":85, "height":64, "ports":[{"connections":[], "x":0, "y":32, "id":"UACANDsxWL"},{"connections":[{"source":"BSlivpgzWn", "target":"8fttHOi8xq", "color":"128,128,255"}], "x":42.5, "y":0, "id":"BSlivpgzWn"},{"connections":[], "x":85, "y":32, "id":"RtJKoO7LO2"},{"connections":[], "x":42.5, "y":64, "id":"OFfF9m3dlQ"}], "annotations":[{"text":"172.16.20.2", "x":913.5, "y":465, "for":"BSlivpgzWn"},{"text":"kali", "x":872, "y":534, "for":"cr9dnaTQdN"}]}]}
    netdef = draw2netdef(json_diagram)
    assert netdef == \
        {'ExperimentDomainName': 'e1.teamname.ncl.sg', 'Nodes': [
            {'Name': 'n1', 'Type': 'Gateway',
    ...
    json_diagram = {"figures":[{"id":"lEfpZPDAcJ", "type":"DiagramFigure", "subtype":"asa", "pic":"icons/asa.png", "x":981, "y":298, "width":59, "height":66, "ports":[{"connections":[{"source":"TzLzMdBsVm", "target":"dCQykgLqKM", "color":"128,128,255"}], "x":0, "y":33, "id":"TzLzMdBsVm"},{"connections":[], "x":29.5, "y":0, "id":"K0qJfIPe7I"},{"connections":[], "x":59, "y":33, "id":"VPNWWLfPJf"},{"connections":[], "x":29.5, "y":66, "id":"c9IXpXDFQ0"},{"connections":[], "x":59, "y":66, "id":"NAs5x4Dc24"},{"connections":[], "x":0, "y":66, "id":"cTJOe8wFBw"},{"connections":[], "x":59, "y":0, "id":"d5pW2VTtS7"},{"connections":[], "x":0, "y":0, "id":"wLC0yeFTVN"}], "annotations":[{"text":"n1", "x":982, "y":369, "for":"lEfpZPDAcJ"}]},{"id":"AIbVnDVrVC", "type":"DiagramFigure", "subtype":"cloud_filled", "pic":"icons/cloud_filled.png", "x":1112, "y":303, "width":84, "height":56, "ports":[{"connections":[{"source":"SwdUa2ZF3p", "target":"VPNWWLfPJf", "color":"128,128,255"}], "x":0, "y":28, "id":"SwdUa2ZF3p"},{"connections":[], "x":42, "y":0, "id":"8XwVq6tcZe"},{"connections":[], "x":84, "y":28, "id":"u83rLhZAZJ"},{"connections":[], "x":42, "y":56, "id":"JcmIX58xv0"},{"connections":[], "x":84, "y":56, "id":"1JOhgwxk4e"},{"connections":[], "x":0, "y":56, "id":"mszJ7MuUlO"},{"connections":[], "x":84, "y":0, "id":"3xZ88pdeVb"},{"connections":[], "x":0, "y":0, "id":"sDGdQecmvP"}], "annotations":[{"text":"e1.teamname; reserve: pc3a, pc4b, pc17e, pc19d", "x":1113, "y":364, "for":"AIbVnDVrVC"}]},{"id":"waSTPwlodp", "type":"DiagramFigure", "subtype":"web_server", "pic":"icons/web_server.png", "x":467, "y":249, "width":53, "height":55, "ports":[{"connections":[], "x":0, "y":27.5, "id":"waC24D9HMF"},{"connections":[], "x":26.5, "y":0, "id":"kbPPhzawpr"},{"connections":[], "x":53, "y":27.5, "id":"KvfzIPL1yF"},{"connections":[], "x":26.5, "y":55, "id":"NKxLHWaVmP"},{"connections":[], "x":53, "y":55, "id":"Akv5SZoITk"},{"connections":[], "x":0, "y":55, "id":"u8mT4sOmTt"},{"connections":[], "x":53, "y":0, "id":"aK6cnsWWFs"},{"connections":[], "x":0, "y":0, "id":"AMBTflAFTB"}], "annotations":[{"text":"win10", "x":468, "y":309, "for":"waSTPwlodp"},{"text":"172.16.1.2", "x":520, "y":276.5, "for":"KvfzIPL1yF"}]},{"id":"AkbPLVpc4r", "type":"DiagramFigure", "subtype":"workstation", "pic":"icons/workstation.png", "x":455, "y":344, "width":64, "height":53, "ports":[{"connections":[], "x":0, "y":26.5, "id":"DhaDHbG63n"},{"connections":[], "x":32, "y":0, "id":"h4k2uhH4LG"},{"connections":[], "x":64, "y":26.5, "id":"SfbuEm5U3Z"},{"connections":[], "x":32, "y":53, "id":"XgrMfguCLx"},{"connections":[], "x":64, "y":53, "id":"5c8SIhbc1w"},{"connections":[], "x":0, "y":53, "id":"NuIkT3ADR4"},{"connections":[], "x":64, "y":0, "id":"yLZMwAKihG"},{"connections":[], "x":0, "y":0, "id":"dnz8IqXW2I"}], "annotations":[{"text":"ubuntu", "x":456, "y":402, "for":"AkbPLVpc4r"},{"text":"172.16.1.3", "x":519, "y":370.5, "for":"SfbuEm5U3Z"}]},{"id":"3IIAhJS92D", "type":"DiagramFigure", "subtype":"workstation", "pic":"icons/workstation.png", "x":633, "y":504, "width":64, "height":53, "ports":[{"connections":[], "x":0, "y":26.5, "id":"sy76SdLSqB"},{"connections":[], "x":32, "y":0, "id":"JLFU7ZdP7P"},{"connections":[], "x":64, "y":26.5, "id":"lmILTLbFGq"},{"connections":[], "x":32, "y":53, "id":"B0AprTpMTO"},{"connections":[], "x":64, "y":53, "id":"SuuvgyzB8q"},{"connections":[], "x":0, "y":53, "id":"hTX8bB8FSc"},{"connections":[], "x":64, "y":0, "id":"FHG6u5zyUi"},{"connections":[], "x":0, "y":0, "id":"MqDH33yvot"}], "annotations":[{"text":"db; install.db", "x":634, "y":562, "for":"3IIAhJS92D"},{"text":"172.16.3.2", "x":665, "y":504, "for":"JLFU7ZdP7P"}]},{"id":"5rbdmr84Q0", "type":"DiagramFigure", "subtype":"workstation", "pic":"icons/workstation.png", "x":762, "y":505, "width":64, "height":53, "ports":[{"connections":[], "x":0, "y":26.5, "id":"p1l5PSKmzI"},{"connections":[], "x":32, "y":0, "id":"khu8TurK0L"},{"connections":[], "x":64, "y":26.5, "id":"S4pJe89i0d"},{"connections":[], "x":32, "y":53, "id":"TMlV4xKdpq"},{"connections":[], "x":64, "y":53, "id":"RlOhqCSygb"},{"connections":[], "x":0, "y":53, "id":"M7mmnGJhqM"},{"connections":[], "x":64, "y":0, "id":"QVs0JpKk6h"},{"connections":[], "x":0, "y":0, "id":"F2vVuuEmkT"}], "annotations":[{"text":"siem; install.splunk", "x":763, "y":563, "for":"5rbdmr84Q0"},{"text":"172.16.3.3", "x":794, "y":505, "for":"khu8TurK0L"}]},{"id":"eCnmGlXr2t", "type":"DiagramFigure", "subtype":"workstation", "pic":"icons/workstation.png", "x":592, "y":108, "width":64, "height":53, "ports":[{"connections":[], "x":0, "y":26.5, "id":"ML8I5E7bAh"},{"connections":[], "x":32, "y":0, "id":"aNhVmV58Ze"},{"connections":[], "x":64, "y":26.5, "id":"22g6dRqTAA"},{"connections":[], "x":32, "y":53, "id":"FXeMW4RIZQ"},{"connections":[], "x":64, "y":53, "id":"EUk8kQOtVd"},{"connections":[], "x":0, "y":53, "id":"sbaU8T7qhZ"},{"connections":[], "x":64, "y":0, "id":"F1Tb1JFtoZ"},{"connections":[], "x":0, "y":0, "id":"cw1UanAR2v"}], "annotations":[{"text":"web; install.web", "x":593, "y":166, "for":"eCnmGlXr2t"},{"text":"172.16.2.2", "x":624, "y":161, "for":"FXeMW4RIZQ"}]},{"id":"Fdd4V6Qoyf", "type":"DiagramFigure", "subtype":"workstation", "pic":"icons/workstation.png", "x":690, "y":110, "width":64, "height":53, "ports":[{"connections":[], "x":0, "y":26.5, "id":"BEeeKQhvlM"},{"connections":[], "x":32, "y":0, "id":"Bg6yRqsANB"},{"connections":[], "x":64, "y":26.5, "id":"sWI6oW7kx0"},{"connections":[], "x":32, "y":53, "id":"lxTUSJSm6D"},{"connections":[], "x":64, "y":53, "id":"qOgRdsEOCf"},{"connections":[], "x":0, "y":53, "id":"BRVKrShI1Q"},{"connections":[], "x":64, "y":0, "id":"v64lqA0nD7"},{"connections":[], "x":0, "y":0, "id":"IOmdxGb9No"}], "annotations":[{"text":"email", "x":691, "y":168, "for":"Fdd4V6Qoyf"},{"text":"172.16.2.3", "x":722, "y":163, "for":"lxTUSJSm6D"}]},{"id":"q0lbQJLT6G", "type":"DiagramFigure", "subtype":"workstation", "pic":"icons/workstation.png", "x":777, "y":108, "width":64, "height":53, "ports":[{"connections":[], "x":0, "y":26.5, "id":"2Vp6MPP0Cn"},{"connections":[], "x":32, "y":0, "id":"RlCnwlc7fT"},{"connections":[], "x":64, "y":26.5, "id":"a9Z6cdqXHS"},{"connections":[], "x":32, "y":53, "id":"7iDKoGfxyq"},{"connections":[], "x":64, "y":53, "id":"ZgAL8vrdvG"},{"connections":[], "x":0, "y":53, "id":"NmfiNxvwgF"},{"connections":[], "x":64, "y":0, "id":"mdObxGD4M9"},{"connections":[], "x":0, "y":0, "id":"EQNHKVAcll"}], "annotations":[{"text":"dns", "x":778, "y":166, "for":"q0lbQJLT6G"},{"text":"172.16.2.4", "x":809, "y":161, "for":"7iDKoGfxyq"}]},{"id":"zDFyeWPdmD", "type":"DiagramFigure", "subtype":"router", "pic":"icons/router.png", "x":687, "y":309, "width":66, "height":45, "ports":[{"connections":[{"source":"06Acbal21m", "target":"KvfzIPL1yF", "color":"128,128,255"},{"source":"06Acbal21m", "target":"SfbuEm5U3Z", "color":"128,128,255"}], "x":0, "y":22.5, "id":"06Acbal21m"},{"connections":[{"source":"Alun6vU20a", "target":"FXeMW4RIZQ", "color":"128,128,255"},{"source":"Alun6vU20a", "target":"lxTUSJSm6D", "color":"128,128,255"},{"source":"Alun6vU20a", "target":"7iDKoGfxyq", "color":"128,128,255"}], "x":33, "y":0, "id":"Alun6vU20a"},{"connections":[{"source":"oPAUnksBvL", "target":"mL9v2Uyvo1", "color":"128,128,255"}], "x":66, "y":22.5, "id":"oPAUnksBvL"},{"connections":[{"source":"2KEiLTRGLR", "target":"JLFU7ZdP7P", "color":"128,128,255"},{"source":"2KEiLTRGLR", "target":"khu8TurK0L", "color":"128,128,255"}], "x":33, "y":45, "id":"2KEiLTRGLR"},{"connections":[], "x":66, "y":45, "id":"HTtoUVwuSo"},{"connections":[], "x":0, "y":45, "id":"lcD5mOIMbI"},{"connections":[], "x":66, "y":0, "id":"BWViFV5pAW"},{"connections":[], "x":0, "y":0, "id":"aaB9g49BDC"}], "annotations":[{"text":"gw; install.snort 172.16.3.3", "x":688, "y":359, "for":"zDFyeWPdmD"},{"text":"172.16.1.100", "x":636, "y":344, "for":"06Acbal21m"},{"text":"172.16.2.100", "x":720, "y":309, "for":"Alun6vU20a"},{"text":"172.16.10.2;NAT:abc", "x":753, "y":332, "for":"oPAUnksBvL"}]},{"id":"gLW5xKF2Oo", "type":"DiagramFigure", "subtype":"router", "pic":"icons/router.png", "x":863, "y":309, "width":66, "height":45, "ports":[{"connections":[], "x":0, "y":22.5, "id":"mL9v2Uyvo1"},{"connections":[], "x":33, "y":0, "id":"P6RlbrJ9of"},{"connections":[], "x":66, "y":22.5, "id":"dCQykgLqKM"},{"connections":[], "x":33, "y":45, "id":"8fttHOi8xq"},{"connections":[], "x":66, "y":45, "id":"yCTHC98TS8"},{"connections":[], "x":0, "y":45, "id":"P9wsoELb6X"},{"connections":[], "x":66, "y":0, "id":"Aw3Xfb7Ug9"},{"connections":[], "x":0, "y":0, "id":"sGzmCT3FSu"}], "annotations":[{"text":"172.16.10.100", "x":863, "y":309, "for":"mL9v2Uyvo1"},{"text":"172.16.20.100", "x":896, "y":354, "for":"8fttHOi8xq"},{"text":"gw2", "x":864, "y":359, "for":"gLW5xKF2Oo"}]},{"id":"cr9dnaTQdN", "type":"DiagramFigure", "subtype":"laptop", "pic":"icons/laptop.png", "x":871, "y":465, "width":85, "height":64, "ports":[{"connections":[], "x":0, "y":32, "id":"UACANDsxWL"},{"connections":[{"source":"BSlivpgzWn", "target":"8fttHOi8xq", "color":"128,128,255"}], "x":42.5, "y":0, "id":"BSlivpgzWn"},{"connections":[], "x":85, "y":32, "id":"RtJKoO7LO2"},{"connections":[], "x":42.5, "y":64, "id":"OFfF9m3dlQ"},{"connections":[], "x":85, "y":64, "id":"NL3UbBU3Zc"},{"connections":[], "x":0, "y":64, "id":"kPOHrSCCoa"},{"connections":[], "x":85, "y":0, "id":"wxNyIZVqRv"},{"connections":[], "x":0, "y":0, "id":"WB1XyT8xBi"}], "annotations":[{"text":"172.16.20.2", "x":913.5, "y":465, "for":"BSlivpgzWn"},{"text":"kali", "x":872, "y":534, "for":"cr9dnaTQdN"}]}]}
...
def main():
    ...
    if (len(sys.argv) == 2) and (sys.argv[1] == "test"):
        ...
        import vagrantfiledebug
        vagrantfiledebug.produce_nsfile(netdef)
        vagrantfiledebug.produce_vf(netdef)
        vagrantfiledebug.produce_ansible(netdef)
        vagrantfiledebug.produce_hostfile(netdef)
        vagrantfiledebug.produce_runcmd(netdef)
        ...
        net_draw = json.loads(file_content)
        netdef = draw2netdef(net_draw)
        print json.dumps(netdef, indent=4)#, separators=(',', ': '))
    elif len(sys.argv) == 3:
        with open(sys.argv[1], 'r') as afile:
            file_content = afile.read()
