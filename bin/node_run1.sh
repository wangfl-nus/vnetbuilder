...
install_virtualbox() {
    ...
    sudo apt-get update
    sudo DEBIAN_FRONTEND=noninteractive apt-get -y install /share/ven/soft/virtualbox/vbox.deb
    yes | sudo DEBIAN_FRONTEND=noninteractive vboxmanage extpack install /share/ven/soft/virtualbox/Oracle_VM_VirtualBox_Extension_Pack-5.2.16.123759.vbox-extpack
...
if ! grep -qs "/proj/$TEAM_NAME/$EXP_NAME/vnetwork/vagrant/boxes" /proc/mounts; then
    if [ ! -d "/proj/$TEAM_NAME/$EXP_NAME/vnetwork/vagrant/boxes" ]; then
